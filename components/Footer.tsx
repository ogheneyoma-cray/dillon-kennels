...
export default function Footer() {
  ...
  return (
    <footer className="bg-aubergine text-oat">
      <div className="container-page flex flex-col items-center py-16 text-center">
        <Logo tone="white" />
        <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-oat/60">
          {site.description}
        </p>

        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
          {LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="font-display text-sm italic text-oat/70 transition-colors hover:text-peach">
              {link.label}
            </Link>
          ))}
        </div>
      ...
      <div className="border-t border-oat/10">
        <div className="container-page flex flex-col items-center gap-3 py-6 text-xs text-oat/40 sm:flex-row sm:justify-between">
          <p>&copy; {year} {site.legalName}. All rights reserved.</p>
          <div className="flex flex-wrap items-center gap-x-5 gap-y-1">
            <a href={`mailto:${site.email}`} className="hover:text-oat">{site.email}</a>
            <a href={`tel:${site.phoneHref}`} className="hover:text-oat">{site.phone}</a>
          </div>
        </div>
      </div>
    </footer>
