...
/**
 * A white sticky header with a thin bottom border, matching the reference's
 * light nav rather than a dark bar — sky-blue accents for the active link
 * and cart badge.
 */
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-ink/10 bg-paper/95 backdrop-blur">
      <div className="hidden border-b border-ink/10 bg-mist py-2 text-center text-[11px] font-semibold uppercase tracking-widest2 text-ink-soft sm:block">
        Instant download after checkout · {site.email}
      </div>
      <div className="container-page flex h-20 items-center justify-between">
        <Link href="/" onClick={() => setMenuOpen(false)} className="shrink-0">
          <Logo wordClassName="text-xl sm:text-2xl" markClassName="h-8 w-8 sm:h-9 sm:w-9" />
        </Link>
            ...
            return (
              <Link
                ...
                className={`relative py-1 text-sm font-bold uppercase tracking-wider transition-colors ${
                  active ? "text-sky-dark" : "text-ink hover:text-sky-dark"
                }`}
              >
                {link.label}
                {active && (
                  <span aria-hidden="true" className="absolute -bottom-1 left-0 h-0.5 w-full rounded-full bg-sky" />
                )}
              </Link>
            );
          })}
