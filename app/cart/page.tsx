    ...
    return (
      <div className="container-page flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 max-w-sm text-ink-soft">
          ...
          marketplace and find something to build with.
        </p>
        ...
        <ul className="divide-y divide-line">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-6 sm:gap-6">
              <Link
                href={`/shop/${item.slug}`}
                className="relative aspect-[4/3] w-24 shrink-0 overflow-hidden rounded-xl bg-lavender sm:w-32"
              >
                <Image
                  src={item.image}
                  alt={item.name}
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link
                      href={`/shop/${item.slug}`}
                      className="font-display text-base font-semibold leading-snug text-ink hover:text-violet"
                    >
                      {item.name}
                    </Link>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center overflow-hidden rounded-xl border border-line">
                    <button
                      ...
                      className="flex h-10 w-10 items-center justify-center text-ink transition-colors hover:bg-lavender"
                    >
                      −
                    </button>
                    <span className="flex h-10 w-10 items-center justify-center border-x border-line text-sm font-semibold">
                      {item.quantity}
                    </span>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="min-h-[44px] px-2 text-sm font-medium text-ink-soft underline underline-offset-4 hover:text-violet"
                  >
                    Remove
                  </button>
                </div>
