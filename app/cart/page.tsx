    ...
    <div className="container-page py-10 lg:py-16">
      ...
      <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_360px]">
        <ul className="divide-y divide-ink/10 border-y border-ink/10">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-6 sm:gap-6">
              <Link
                href={`/shop/${item.slug}`}
                className="relative h-24 w-20 shrink-0 overflow-hidden bg-stone sm:h-32 sm:w-28"
              >
                <Image
                  src={item.image}
                  alt={item.name}
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link
                      href={`/shop/${item.slug}`}
                      className="font-display text-lg uppercase leading-snug text-ink hover:text-spruce sm:text-xl"
                    >
                      {item.name}
                    </Link>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center border border-ink/20">
                    <button
                      ...
                      className="flex h-10 w-10 items-center justify-center text-ink transition-colors hover:bg-stone"
                    >
                      −
                    </button>
                    <span className="flex h-10 w-10 items-center justify-center border-x border-ink/20 text-sm font-bold">
                      {item.quantity}
                    </span>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="min-h-[44px] px-2 text-sm font-semibold text-ink/60 underline underline-offset-4 hover:text-spruce"
                  >
                    Remove
                  </button>
                </div>
        ...
        <aside className="h-fit border border-ink/10 bg-stone p-6">
          ...
          <div className="mt-5 flex justify-between border-t border-ink/10 pt-5 font-display text-xl uppercase text-ink">
            <span>Total</span>
            <span>{formatMoney(cartTotal, currency)}</span>
          </div>
