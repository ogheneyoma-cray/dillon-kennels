    ...
    return (
      <div className="container-page flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 max-w-sm text-ink-soft">
          You haven&apos;t added anything to your cart yet. Explore the shop
        ...
        <ul className="divide-y divide-line border-y border-line">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-6 sm:gap-6">
              <Link
                href={`/shop/${item.slug}`}
                className="relative aspect-[4/5] w-20 shrink-0 overflow-hidden rounded-lg bg-sand sm:w-28"
              >
                <ProductCover
                  src={item.image}
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link
                      href={`/shop/${item.slug}`}
                      className="font-display text-base leading-snug text-ink hover:text-clay sm:text-lg"
                    >
                      {item.name}
                    </Link>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="min-h-[44px] px-2 text-sm font-medium text-ink-soft underline underline-offset-4 hover:text-clay"
                  >
                    Remove
                  </button>
                </div>
