    ...
    return (
      <div className="container-page flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 max-w-sm text-ink-soft">
          ...
          store and find your next build.
        </p>
        ...
        <ul className="divide-y divide-line border-y border-line">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-6 sm:gap-6">
              <Link
                href={`/shop/${item.slug}`}
                className="relative aspect-[4/5] w-20 shrink-0 overflow-hidden rounded-xl bg-silver-pale sm:w-28"
              >
                <ProductCover
                  src={item.image}
                  alt={`${item.name} cover`}
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link
                      href={`/shop/${item.slug}`}
                      className="font-display text-base leading-snug text-ink hover:text-fuchsia sm:text-lg"
                    >
                      {item.name}
                    </Link>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center rounded-full border border-ink/15">
                    <button
                      ...
                      className="flex h-10 w-10 items-center justify-center text-ink transition-colors hover:bg-silver-pale"
                    >
                      −
                    </button>
                  ...
                  <button
                    ...
                    className="min-h-[44px] px-2 text-sm font-medium text-ink-soft underline underline-offset-4 hover:text-fuchsia"
                  >
                    Remove
                  </button>
                </div>
