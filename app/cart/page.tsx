...
export default function CartPage() {
    ...
    return (
      <div className="container-page flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 max-w-sm text-ink/70">
          You haven't added any products to your cart yet. Browse the shop and
  ...
  return (
    <div className="container-page py-10 lg:py-16">
      <p className="eyebrow">Your Cart</p>
      <h1 className="section-heading mt-3">Cart</h1>
        ...
        <ul className="divide-y divide-ink/10 border-y border-ink/10">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-6 sm:gap-6">
              <Link
                href={`/shop/${item.slug}`}
                className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-ink sm:h-28 sm:w-28"
              >
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex justify-between gap-3">
                  <div>
                    <Link
                      href={`/shop/${item.slug}`}
                      className="font-display text-base font-semibold leading-snug text-ink hover:text-teal-dark sm:text-lg"
                    >
                      {item.name}
                    </Link>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center rounded-full border border-ink/20">
                    <button
                      ...
                      className="flex h-10 w-10 items-center justify-center rounded-full text-ink transition-colors hover:bg-teal-pale"
                    >
                      −
                    </button>
                  ...
                  <button
                    ...
                    className="min-h-[44px] px-2 text-sm font-semibold text-ink/60 underline underline-offset-4 hover:text-teal-dark"
                  >
                    Remove
                  </button>
                </div>
