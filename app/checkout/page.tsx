...
export default function CheckoutPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="boxed flex flex-col items-center py-28 text-center">
        ...
        <p className="mt-4 max-w-md text-[15px] text-mudd">
  ...
  return (
    <div className="boxed py-14">
      <h1 className="head-xl text-pine">Where are we sending it?</h1>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_360px]">
        <form onSubmit={handleSubmit} noValidate>
            ...
            {FIELDS.map((field) => (
              <div key={field.name} className={field.full ? "sm:col-span-2" : ""}>
                ...
                {errors[field.name] && (
                  <p
                    id={`${field.name}-error`}
                    className="mt-1.5 text-[12px] font-bold text-tangerine-dark"
                  >
                    {errors[field.name]}
                  </p>
                )}
          ...
          <p className="mt-6 border-l-4 border-lagoon bg-lagoon/10 px-4 py-3.5 text-[13px] leading-relaxed text-forest">
            This is a demonstration storefront. No payment is taken and no card
            details are collected at any point.
          </p>

          <button type="submit" className="btn-solid mt-7 w-full sm:w-auto">
            Place order
          </button>
        </form>

        <aside className="h-fit border border-pine/10 bg-white p-7 lg:sticky lg:top-28">
          <h2 className="head-sm text-pine">Your order</h2>

          <ul className="mt-6 space-y-4">
            {items.map((item) => (
              <li key={item.id} className="flex items-center gap-3">
                <span className="relative h-16 w-12 shrink-0 overflow-hidden bg-shell">
                  <Image
                    src={item.image}
                    alt=""
                    fill
                ...
                <span className="flex-1 text-[13px] leading-snug text-pine">
                  {item.name}
                  <span className="mt-0.5 block text-mudd">
                    Qty {item.quantity}
                  </span>
                </span>
          ...
          <dl className="mt-6 space-y-3 border-t border-pine/10 pt-5 text-sm">
            ...
            <div className="flex justify-between border-t border-pine/10 pt-3 text-base font-extrabold">
              <dt>Total</dt>
